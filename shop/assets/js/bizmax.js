...
function getFormObject(form_name) {
  ...
  var form_object = {};
  jQuery.each(form_serialize, function () {
...
function saveFormPrgsAreaCreate(fn_param) {
  ...
  $("div[name='" + sf_area_div_name + "'] button[name='" + save_btn_name + "']")
    .text(save_btn_lock_text)
    .prop("disabled", true);
...
function onFormFileSelChg(fn_param) {
  ...
  var fileValue = $("form[name='" + form_name + "'] [name='" + clm_name + "']")
    .val()
    .split("\\");
...
$(function () {
  ...
  $(document).ajaxSend(function (event, jqXHR, ajaxOptions) {
    if (hasValue(ajaxOptions["sapcs"])) {
      //stand alone process 독립프로세스 처리안함
      return;
    }
