...
function fontResponsive($fontsize, $max, $size1, $size2, $size3) {
  if ($(window).innerWidth() < $max && $(window).innerWidth() > $size1) {
    $(document)
      .find("html")
      .animate({ fontSize: ($(window).innerWidth() * 10) / $max }, 0);
  } else if ($(window).innerWidth() < $size2 && $(window).innerWidth() > $size3) {
    $(document)
      .find("html")
      .animate({ fontSize: ($(window).innerWidth() * 19) / $max }, 0);
  } else if ($(window).innerWidth() <= $size3) {
    $(document)
      .find("html")
      .css({ fontSize: $fontsize / 1.5 + "px" });
  } else {
    $(document)
      .find("html")
  ...
  (b.lineCap = i),
    (t = setInterval(function () {
      (I = o ? --I : (I += 1)),
        ...
        (P = o ? 100 - (I / -M) * 100 : (I / M) * 100),
        S.text(Math.floor(P)),
        ((I <= -(a - 1) && o) || (a <= I && !o)) && clearInterval(t);
    }, v));
};
