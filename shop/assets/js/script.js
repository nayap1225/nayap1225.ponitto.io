$(function () {
  ...
  $(".sort_list_box .dropdown_list").on("click", "button", function () {
    var $this = $(this);
    ($parent = $this.parent("li")), ($wrap = $this.closest(".prod_sort_area")), ($items = $wrap.find(".dropdown_list")), ($dropdownBtn = $wrap.find(".btn_dropdown")), ($selectText = $wrap.find(".selected_text"));
    _text = $this.text();
...
function masonryLayout() {
  const gridItemsStyle = getComputedStyle(document.querySelector(".mansonry_grid .grid_items"));
  const columnGap = parseInt(gridItemsStyle.getPropertyValue("column-gap"));
  const autoRows = parseInt(gridItemsStyle.getPropertyValue("grid-auto-rows"));
  document.querySelectorAll(".mansonry_grid .item").forEach((elt) => {
    elt.style.gridRowEnd = `span ${Math.ceil(elt.querySelector(".item_inner").scrollHeight / autoRows + columnGap / autoRows + 2)}`;
  });
}
...
function layerPopupScroll(e) {
  ...
  (sl = $(window).scrollLeft()), (ww = $(window).width()), (tw = $target.outerWidth());
